 ...
 * LegalNotice.vue
 * 
 * Página contenedora de Aviso Legal — LABANDA Industrie Services.
 * Carga dinámicamente el contenido según el idioma del usuario.
...
import { LegalNoticeContents } from './index'

const { locale } = useI18n()

const CurrentContent = computed(() => {
  return LegalNoticeContents[locale.value as keyof typeof LegalNoticeContents] || LegalNoticeContents.es
})
</script>
...
<template>
  <main class="min-h-screen bg-white text-gray-800">
    ...
    <section class="bg-[#003366] text-white py-16 px-6 text-center">
      <h1 class="text-4xl font-bold mb-2">{{ $t('legal.titles.notice') }}</h1>
      <p class="text-blue-100 text-lg">{{ $t('legal.subtitles.notice') }}</p>
    </section>
